...
# Цей endpoint повертає курси відносно USD
...
# --- Допоміжна Функція: Пошук ID за Символом (без змін) ---
def find_coin_id(symbol: str) -> str | None:
    # ... (код find_coin_id залишається незмінним) ...
...
# --- Допоміжна Функція: Отримання Фіатних Курсів (ВИПРАВЛЕНО) ---
def get_fiat_rates() -> dict | None:
    ...
    try:
        ...
        data = response.json()
        # Перевіряємо статус успіху та наявність словника 'rates'
        if data.get('result') == 'success' and 'rates' in data:
            return data.get('rates')

        # Якщо API повертає іншу структуру, але має 'rates', беремо її
        if 'rates' in data:
            return data.get('rates')
...
# --- Основна Функція: Отримання Ціни (ВИПРАВЛЕНО) ---
def get_crypto_price(crypto_symbol: str) -> tuple[str, str] | tuple[None, str]:
    ...
    if symbol in ["EUR", "USD"]:
        ...
        # Перевірка наявності потрібних курсів
        ...
        try:
            ...
            # Якщо курс EUR_per_USD=0 (тобто 1 USD в EUR), це означає помилку.
            if uah_per_usd == 0 or eur_per_usd == 0:
                return None, "Фіатні курси недійсні (значення = 0)."

            if symbol == "USD":
                usd_price = 1.0
                # Курс USD/UAH = UAH_per_USD / 1 USD
                ...
                # Конверсія: 1 EUR в USD = 1 / EUR_per_USD
                # Оскільки API повертає скільки EUR коштує 1 USD, нам треба інвертувати
                usd_price = 1.0 / eur_per_usd
                # Конверсія: 1 EUR в UAH = (1 EUR в USD) * (USD в UAH)
    ...
    # ... (Весь код обробки крипти залишається без змін) ...
